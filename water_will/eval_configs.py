...
def main():
    ...
    for batch in data_loader:
        ...
        output = model(
            batch.to_dict(), compute_stress=args.compute_stress, compute_force=True
        )
    ...
    assert len(atoms_list) == len(energies) == len(forces_list) == len(density_coefficients_list)
    ...
    for i, (atoms, energy, forces, density_coefficients) in enumerate(
        zip(atoms_list, energies, forces_list, density_coefficients_list)
    ):
        atoms.calc = None  # crucial
        atoms.info[args.info_prefix + "energy"] = energy
        atoms.arrays[args.info_prefix + "forces"] = forces
        atoms.arrays[args.info_prefix + "mace_charges"] = density_coefficients
        # atoms.info[args.info_prefix + "dipole"] = dipole[0]
