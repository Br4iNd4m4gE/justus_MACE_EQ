...
def train(
    ...
    scf_training_options: Optional[Dict[str, Any]] = None,
    ...
    while epoch < max_num_epochs:
        ...
        if epoch % eval_interval == 0:
            ...
            elif log_errors == "EFQRMSE":
                error_e = eval_metrics["mae_e_per_atom"] * 1e3
                error_f = eval_metrics["mae_f"] * 1e3
                error_q = eval_metrics["rmse_q"]
                logging.info(f"Epoch {epoch}: loss={valid_loss:.7f}, RMSE_q = {error_q:.7f}, MAE_E_per_atom={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A")
...
def take_step(
    ...
    if scf_training_options is not None:
        output = model(
            batch_dict,
            training=True,
...
def evaluate(
    ...
    if charges_computed:
        delta_q = to_numpy(torch.cat(delta_charges_list,dim=0))
        aux["rmse_q"] = compute_rmse(delta_q)
    if dmas_computed:
