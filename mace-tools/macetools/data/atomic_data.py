...
class AtomicData(torch_geometric.data.Data):
    ...
    def __init__(
        ...
        esp: Optional[torch.Tensor],  # [n_nodes, ]
        esp_gradient: Optional[torch.Tensor],  # [n_nodes, 3]
        density_coefficients: Optional[torch.Tensor],  # [n_nodes, ...]
        density_coefficients_weight: Optional[torch.Tensor],  # [,]
        charges_weight: Optional[torch.Tensor],
        rcell: Optional[torch.Tensor],  # [3,3]
        volume: Optional[torch.Tensor],  # [1]
        pbc: Optional[torch.Tensor],  # [3,1]
        total_charge: Optional[torch.Tensor],  # [1]
        external_field: Optional[torch.Tensor],  # [3]
        fermi_level: Optional[torch.Tensor],  # [1]
    ):
        # Check shapes
        ...
        assert dipole_weight is None or len(dipole_weight.shape) == 0
        ...
        assert esp is None or esp.shape == (num_nodes,)
        assert esp_gradient is None or esp_gradient.shape == (num_nodes, 3)
        assert (
        ...
        data = {
            ...
            "density_coefficients": density_coefficients,  # added stuff below
    ...
    def from_config(
        ...
        edge_index, shifts, unit_shifts, fixed_cell = get_neighborhood(
        ...
        cell = torch.tensor(fixed_cell, dtype=torch.get_default_dtype())
        ...
        esp = (
            torch.tensor(config.esp, dtype=torch.get_default_dtype())
            if config.esp is not None
            else None
        )
        esp_gradient = (
            torch.tensor(config.esp_gradient, dtype=torch.get_default_dtype())
            if config.esp_gradient is not None
            else None
        )
        charges_weight = (
            torch.tensor(
                config.charges_weight,dtype=torch.get_default_dtype()
            )
            if config.charges_weight is not None 
            else 1
        )
        density_coefficients = (
            torch.tensor(config.density_coefficients, dtype=torch.get_default_dtype())
            if config.density_coefficients is not None
            else None
        )
        density_coefficients_weight = (
            torch.tensor(
                config.density_coefficients_weight, dtype=torch.get_default_dtype()
            )
            if config.density_coefficients_weight is not None
            else 1
        )
        volume = torch.linalg.det(cell) if cell is not None else None
        rcell = (
            2 * pi * torch.linalg.inv(cell.mT)
            if volume > 0
            else torch.tensor(
        ...
        external_field = (
            torch.tensor(config.external_field, dtype=torch.get_default_dtype())
            if config.external_field is not None
            else torch.tensor([3 * [0.0]], dtype=torch.get_default_dtype())
        )
        fermi_level = (
            torch.tensor(config.fermi_level, dtype=torch.get_default_dtype())
            if config.fermi_level is not None
