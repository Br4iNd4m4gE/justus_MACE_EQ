...
def main():
    ...
    assert len(atoms_list) == len(energies) == len(forces_list) == len(density_coefficients_list) == len(dipoles)
    ...
    for i, (atoms, energy, forces, density_coefficients, dipole) in enumerate(
        zip(atoms_list, energies, forces_list, density_coefficients_list, dipoles)
    ):
