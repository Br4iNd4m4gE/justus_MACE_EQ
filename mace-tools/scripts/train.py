...
from macetools.electrostatics.qeq import LQEqMACE
import ast
import logging
...
from macetools.utils.script_utils import model_enum
...
def main():
    ...
    tag = tools.get_tag(name=args.name, seed=args.seed)
    # TODO: implement a schema validator using pydantic

    # now we need to refactor the rest of run_train such that we don't have to change things in multiple places
    # tag = tools.get_tag(name=args.name, seed=args.seed)
